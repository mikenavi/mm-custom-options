...
Формат JSON не поддерживает комментарии, тем не менее можно использовать следующий синтаксис для комментриварония значений внутри объекта настроек не теряя при этом в ф ункциональности:
