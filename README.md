...
## `hp-event` :octopus:
Содержит одно или несколько значений ключа `values`, единичный выбор значения в группе.
Может быть паретном, а также сам являться парентом наравно с другими опциями.

`hpId` - тип: строка
- ID товара на платформе

**Внимание, не может быть носителем собсвенной "цены", цену опции эвента можено задавать через парента (можно скрытого). Данная механика находится в процессе реализации**
